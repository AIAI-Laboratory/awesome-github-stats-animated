...
import { getUfoTheme, getUfoThemeCss } from '../../src/githubStatsTheme.js';

const LEVEL_OPACITY = [0.08, 0.32, 0.52, 0.75, 1];

function renderDays(days, accent, text) {
  return days.map((day, index) => {
    ...
    return `<rect class="cell" x="${x}" y="${y}" width="10" height="10" rx="2" fill="${level === 0 ? text : accent}" opacity="${LEVEL_OPACITY[level]}" style="--delay:${(index * 0.004).toFixed(3)}s"><title>${escapeXml(day.date)} · ${day.count} contributions</title></rect>`;
  }).join('');
}
...
export function render(input) {
  ...
  const palette = getUfoTheme(theme, options.accent);
  const themeCss = getUfoThemeCss(theme, options.accent);
  const name = escapeXml(truncate(profile.name, 30));
  const handle = escapeXml(truncate(profile.login, 39));
  const body = `
    <rect width="980" height="320" rx="24" fill="${palette.background}"/>
    <rect x="1" y="1" width="978" height="318" rx="23" fill="none" stroke="${palette.divider}" stroke-width="2"/>
    ...
    <path d="M48 132H932" stroke="${palette.divider}"/>
    ${renderDays(calendar.days, palette.accent, palette.text)}
  ...
  return svgDocument({
    ...
    styles: `
      ...
      .kicker,.label{fill:${palette.accent};font-size:9px;font-weight:700;letter-spacing:1.8px}
      .name{fill:${palette.text};font-size:28px;font-weight:750}.value{fill:${palette.text};font-size:30px;font-weight:750}
      .muted{fill:${palette.muted};font-size:11px}.cell{transform-box:fill-box;transform-origin:center;animation:enter .5s ease-out var(--delay) both}
